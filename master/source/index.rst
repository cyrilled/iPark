.. iPark documentation master file, created by Cyrille Desmoulins 6/6/2023
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

iPark
=============

Ce site regroupe l'ensemble des comptes rendus de réunion ayant eu lieu jusqu'à présent dans le cadre du projet iPark. Ce projet concerne la gestion des accès à des parking payants. L'objectif du projet est de développer le logiciel iPark dans le cadre d'un consortium mettant en oeuvre différentes parties prenantes au sein de différentes organisations (à ce jour, IM2AG, AccessIt, Centre hospitalier universitaire Grenoble-Alpes et Z-Park).

Comptes rendus de réunions
--------------------------

.. toctree::
    :maxdepth: 1

    res/CR001.rst
    res/CR002.rst
    res/CR003.rst
    res/CR004.rst
    res/DocDrivers.rst

